...
ajaxMore()
	...
	Barba.Dispatcher.on('newPageReady', function() {
        ...
        if(document.getElementById('formulaire')) {
            $.getScript('js/contact-form-7/scripts.js');
        }
	});
	...
	function NewsToNewsTransition(e) {
		...
		if (Barba.HistoryManager.prevStatus().namespace == 'index.html') { 
			TweenMax.to('video', 1, {
				yPercent: -40,
				ease: Power3.easeInOut,
				onComplete: function() {
...
$("#modal-logon").on('click', 'header a', function(event) {
    ...
    if( $this.index() === 0 ){
        $("#modal-logon .iziModal-content .icon-close").css('background', '#ddd');
    } else {
        $("#modal-logon .iziModal-content .icon-close").attr('style', '');
    }
});

$("#modal-logon").on('click', '.submit', function(event) {
    event.preventDefault();
...
$("#modal-logon").iziModal({
    history: false,
    overlayClose: true,
    width: 600,
    overlayColor: 'rgba(19, 41, 75, 0.6)',
    ...
    onOpened: function() {
		...
		$('#modal-underConstruction').iziModal('open');
    },
    onClosed: function() {
...
$("#modal-alert").iziModal({
    title: "This is a test confirmation.",
    icon: 'icon-check',
    headerColor: '#00af66',
    width: 600,
    timeout: 10000,
...
$("#modal-alert2").iziModal({
    title: "Attention",
    subtitle: 'This is an error alert!',
...
$("#modal-underConstruction").iziModal({
    title: "Under Construction",
    subtitle: 'We are sorry but this feature is missing or still under construction.  Please check back soon.',
    icon: 'icon-power_settings_new',
	headerColor: '#886bbd',
    width: 525,
    timeout: 5000,
...
$(document).on('click', '.trigger-logon', function (event) {
	event.preventDefault();
    // $('#modal-logon').iziModal('setZindex', 1000);
    // $('#modal-logon').iziModal('open', { zindex: 1000 });
    $('#modal-logon').iziModal('open');
});
...
$(document).on('click', '.trigger-underConstruction', function (event) {
    event.preventDefault();
    $('#modal-underConstruction').iziModal('open');
});

$(document).on('click', '.trigger-yelp1', function (event) {
    event.preventDefault();
    $('#modal-yelp1').iziModal('open');
});

$(document).on('click', '.trigger-yelp2', function (event) {
    event.preventDefault();
    $('#modal-yelp2').iziModal('open');
});
